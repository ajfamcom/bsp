...
function twenty_twenty_one_continue_reading_text() {
	$continue_reading = sprintf(
		...
		esc_html__( 'Continue reading %s', 'bspmain' ),
...
if ( ! function_exists( 'twenty_twenty_one_post_title' ) ) {
	...
	function twenty_twenty_one_post_title( $title ) {
		return '' === $title ? esc_html_x( 'Untitled', 'Added to posts and pages that are missing titles', 'bspmain' ) : $title;
	}
}
...
function twenty_twenty_one_password_form( $output, $post = 0 ) {
	...
	$output = '<p class="post-password-message">' . esc_html__( 'This content is password protected. Please enter a password to view.', 'bspmain' ) . '</p>
	<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
	<label class="post-password-form__label" for="' . esc_attr( $label ) . '">' . esc_html_x( 'Password', 'Post password form', 'bspmain' ) . '</label><input class="post-password-form__input" name="post_password" id="' . esc_attr( $label ) . '" type="password" spellcheck="false" size="20" /><input type="submit" class="post-password-form__submit" name="' . esc_attr_x( 'Submit', 'Post password form', 'bspmain' ) . '" value="' . esc_attr_x( 'Enter', 'Post password form', 'bspmain' ) . '" /></form>
	';
	return $output;
}
