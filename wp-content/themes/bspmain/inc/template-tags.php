...
if ( ! function_exists( 'twenty_twenty_one_posted_on' ) ) {
	...
	function twenty_twenty_one_posted_on() {
		...
		printf(
			...
			esc_html__( 'Published %s', 'bspmain' ),
...
if ( ! function_exists( 'twenty_twenty_one_entry_meta_footer' ) ) {
	...
	function twenty_twenty_one_entry_meta_footer() {
		...
		if ( ! is_single() ) {
			...
			if ( is_sticky() ) {
				echo '<p>' . esc_html_x( 'Featured post', 'Label for sticky posts', 'bspmain' ) . '</p>';
			}
			...
			if ( has_category() || has_tag() ) {
				...
				if ( $categories_list ) {
					printf(
						...
						'<span class="cat-links">' . esc_html__( 'Categorized as %s', 'bspmain' ) . ' </span>',
				...
				if ( $tags_list ) {
					printf(
						...
						'<span class="tags-links">' . esc_html__( 'Tagged %s', 'bspmain' ) . '</span>',
...
if ( ! function_exists( 'twenty_twenty_one_the_posts_navigation' ) ) {
	...
	function twenty_twenty_one_the_posts_navigation() {
		the_posts_pagination(
			array(
				'before_page_number' => esc_html__( 'Page', 'bspmain' ) . ' ',
				'mid_size'           => 0,
				'prev_text'          => sprintf(
					'%s <span class="nav-prev-text">%s</span>',
					is_rtl() ? twenty_twenty_one_get_icon_svg( 'ui', 'arrow_right' ) : twenty_twenty_one_get_icon_svg( 'ui', 'arrow_left' ),
					wp_kses(
						__( 'Newer <span class="nav-short">posts</span>', 'bspmain' ),
						array(
							'span' => array(
								'class' => array(),
							),
				...
				'next_text'          => sprintf(
					...
					wp_kses(
						__( 'Older <span class="nav-short">posts</span>', 'bspmain' ),
						array(
							'span' => array(
								'class' => array(),
							),
