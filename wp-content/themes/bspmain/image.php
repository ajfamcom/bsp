...
while ( have_posts() ) {
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		...
		<div class="entry-content">
			...
			wp_link_pages(
				array(
					'before'   => '<nav class="page-links" aria-label="' . esc_attr__( 'Page', 'bspmain' ) . '">',
					'after'    => '</nav>',
					/* translators: %: Page number. */
					'pagelink' => esc_html__( 'Page %', 'bspmain' ),
				)
			);
			?>
		</div><!-- .entry-content -->
		...
		<footer class="entry-footer default-max-width">
			...
			if ( wp_get_post_parent_id( $post ) ) {
				...
				printf(
					...
					esc_html__( 'Published in %s', 'bspmain' ),
			...
			if ( $metadata ) {
				printf(
					...
					esc_html_x( 'Full size', 'Used before full size attachment link.', 'bspmain' ), // phpcs:ignore WordPress.Security.EscapeOutput
