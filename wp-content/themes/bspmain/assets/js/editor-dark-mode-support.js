/* global bspmainIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'bspmain-supports-dark-theme' ) ) {
	bspmainDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function bspmainDarkModeEditorInit( attempt ) {
	...
	if ( bspmainIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						bspmainDarkModeEditorInit( attempt + 1 );
					},
